...
Запрашивать статус игры, в ответ получает, кончилась ли игра, чей ход, какие карты у игрока на руках.
...
class Game():
    ...
    def move(self, token):
        """"
        если клиент получает в запросе информацию, что он теперь ходит, 
        он может отправить запрос на следующий ход. 
        
        """
        pass
        
    def get_state (self, gameID, token):
        """это запрос о текущем состоянии игры, с данными которые передаются игроку
        Передает следующие параметры:
            - если игра не найдена - сообщить
            - если игра найдена, но токен игрока неверный - сообщить
            - game_over: False - если игра идет, True - если закончилась
            - winners: если игра закончена, то передает json, в котором указано,
            кто победитель и статус каждого участника - Имя, жив/мертв, 
            заражен/человек/нечто, иначе None
            - если ход данного игрока, то передать json c данными:
                - карта, полученная из сброса
                - карты на руках
                - статус игрока - человек, нечто, зараженный
                - кто ходит следующим
                - направление хода
                - доступные ходы
            - если ход другого игрока, сообщить:
                - чей ход
                - свой статус
                - карты на руках
                - направление хода
